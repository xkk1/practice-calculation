...
def update_data():
    global data
    # 个人信息更新
    now_question_detail = f'{question_form[0]}{question_form[1]}{question_form[2]} {order_of_units} {question_number}'
...
“+-*/”是随机两数相加或相减或相乘或相除'''
    ...
    def show_best():
        ...
        if data['best'] == []:
            ...
            for form in data['best']:
                information += f"账号:{data['best'][form][0]},类型:{form}正确数:{data['best'][form][1]},所用时间:{data['best'][form][2]}\n"
    ...
    # scr.place(x=50, y=50) #滚动文本框在页面的位置
    statistics_scrolledtext.pack(fill=tk.BOTH, padx=5, pady=5)
